...
def filename = "T19_16404_I6_DAPI.ome.tif"
def downsample = 1
def proj = QP.getProject()

def singleChannelServer = new TransformedServerBuilder(QP.getCurrentServer() as ImageServer<BufferedImage>).extractChannels("DAPI").build()

// Output server path
def path = buildFilePath(PROJECT_BASE_DIR, filename)

// Create a scaling & bit-depth-clipping server
def server = new TypeConvertServer(singleChannelServer, 100f, 0f)
